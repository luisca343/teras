...
**§4.5 Contradictory / dead methods in `Teras`** — [Teras.java](../src/main/java/es/boffmedia/teras/Teras.java), [ClientProxy.java](../src/main/java/es/boffmedia/teras/client/ClientProxy.java)
- Caller review showed the SmartRotom pad uses a **whitelist** (only the smartrotom site is permitted; any other URL is redirected to the pad's home). The body of `isSiteBlacklisted` (`!url.contains("smartrotom")`) was correct for that one caller, but the *name* was the logical inverse of its behavior. Renamed to the positive `isSiteAllowed(url)` (`url.contains("smartrotom")`) + documented + inverted the single caller in `ClientProxy.onAddressChange`.
- Deleted `getNextAvailablePadID()` (always returned `0`, **zero callers**) and the empty no-op `setBackup()` (all five call sites were already commented out).

**§4.6 `TerasBattleOld` — renamed (not merged); the audit premise was inaccurate** — [BattleSession.java](../src/main/java/es/boffmedia/teras/pixelmon/battle/BattleSession.java), [CombateFrenteBatalla.java](../src/main/java/es/boffmedia/teras/pixelmon/battle/CombateFrenteBatalla.java), [TerasBattleEvent.java](../src/main/java/es/boffmedia/teras/event/TerasBattleEvent.java)
- Inspection showed `TerasBattleOld` is **not** a duplicate of `TerasBattle`: `TerasBattle` wraps a `BattleController` for battle log/state observation (used by the ~16 action handlers), whereas `TerasBattleOld` is a battle **setup/launcher** (holds `player`/`battleConfig`/`entity`, has `start()`, level calc, rival-participant building) and is the superclass of `CombateFrenteBatalla`. They are distinct concerns, so a merge was the wrong fix.
- Renamed `TerasBattleOld` → **`BattleSession`** (3 dependent files updated) to remove the misleading "Old" suffix, and stripped ~93 lines of dead commented-out alternative implementations (the superseded team/position-map `swapPokemon`/`getStats`/`setStats`/`getActivePokemon`/`getActiveTeam`/`getPositionString` blocks). 484 → 391 lines.

### ⏳ Not yet started
- *(No tracked items from the original §24 plan remain open. Remaining audit recommendations — tests/CI, `WorldSavedData`/Capability migration, i18n, HUD, cross-mod integrations — are 30/90-day roadmap items, not remediation backlog.)*
