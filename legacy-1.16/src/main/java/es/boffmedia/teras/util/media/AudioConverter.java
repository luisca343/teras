...
public class AudioConverter {
    ...
    private static short[] convert(AudioInputStream source) throws IOException {
        ...
        return TerasVoicechatPlugin.SERVER_API.getAudioConverter().bytesToShorts(IOUtils.toByteArray(stream2));
    }

    public static short[] convertMp3(Path file) throws IOException, UnsupportedAudioFileException {
        Mp3Decoder mp3Decoder = TerasVoicechatPlugin.SERVER_API.createMp3Decoder(Files.newInputStream(file));
        if (mp3Decoder == null) {
            throw new IOException("Error creating mp3 decoder");
        }
        byte[] data = TerasVoicechatPlugin.SERVER_API.getAudioConverter().shortsToBytes(mp3Decoder.decode());
